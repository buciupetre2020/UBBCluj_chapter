...
G <- df[,dict[dict$category %in% c("ID",'G'), 1, drop=TRUE]]
W1 <- df[df$wave=='W1',dict[dict$category %in% c('ID','W'), 1, drop=TRUE]]
W2 <- df[df$wave=='W2',dict[dict$category %in% c("ID",'W'), 1, drop=TRUE]]
W3 <- df[df$wave=='W3',dict[dict$category %in% c("ID", "W"), 1, drop=TRUE]]
DEMOG <- df[,dict[dict$category %in% c("ID", 'DEMOG'), 1, drop=TRUE]]

G <- G[,1:12]
W1 <- W1[, names(W1) %in% dict[dict$var==1,1,drop=TRUE]]
W2 <- W2[, names(W2) %in% dict[dict$var==1,1,drop=TRUE]]
W3 <- W3[, names(W3) %in% dict[dict$var==1,1,drop=TRUE]]

W1 <- W1[, W1 %>% apply(., MARGIN = 2, function(x) mean(is.na(x)) < 0.1)] |> drop_na()
W2 <- W2[, W2 %>% apply(., MARGIN = 2, function(x) mean(is.na(x)) < 0.1)] |> drop_na()
W3 <- W3[, W3 %>% apply(., MARGIN = 2, function(x) mean(is.na(x)) < 0.1)] |> drop_na()


model <- randomForest::randomForest(G[,6:12] |> 
